...
old_dir_dqn = CURRENT_DIR.parent / "trains" / "deathmatch" / "dqn-Seba-v3-2"
old_dir_ppo = CURRENT_DIR.parent / "trains" / "deathmatch" / "ppo-Seba-v3-2"
...
class RewardShapingWrapper(RewardWrapper):
    def __init__(self, env, hit_miss_penalty=-0.001, hit_reward=10, attack_ready_penalty=-1): 

        super(RewardShapingWrapper, self).__init__(env)

        # self.damage_reward = damage_reward
        self.hit_miss_penalty = hit_miss_penalty
        self.hit_reward = hit_reward
        self.attack_ready_penalty = attack_ready_penalty
    ...
    def reset(self, **kwargs):
        ...
        self.previous_hitcount = game_variables[1]  # HITCOUNT
        self.previous_ammo = game_variables[2]  # AMMO0

        print(f"Game variables: {game_variables}")
        print(f"Reset: Hitcount: {self.previous_hitcount}, Ammo: {self.previous_ammo}")
    ...
    def reward(self, reward):
        ...
        if game_state:
            ...
            current_attack_ready = game_variables[0]  # ATTACK_READY
            current_hitcount = game_variables[1]  # HITCOUNT
            current_ammo = game_variables[2]  # AMMO0
            current_hit_miss = (self.previous_ammo-current_ammo)-(current_hitcount-self.previous_hitcount)
            

            if(current_attack_ready):
                custom_reward += self.attack_ready_penalty

            if current_hit_miss > 0:
                reward_gain = current_hit_miss*self.hit_miss_penalty
            ...
            if current_hitcount > self.previous_hitcount:
                ...
                custom_reward += reward_gain
                print(f"Recompensa por hacer daño: {reward_gain}, Hits hechos: {hitcount_delta}, Recompensa actual: {custom_reward}, Hits perdidos: {current_hit_miss}, current ammo: {current_ammo}, previous ammo: {self.previous_ammo}")
